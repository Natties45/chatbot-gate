...
async function runGitCmd(cmd: string, cwd: string): Promise<{ stdout: string; stderr: string; error?: any }> {
  try {
...
export async function executeGitAction(
  ...
  const settings = await getSettings();
  const localPath = settings['git.localPath'] || '/root/openstack-support';
  let repoUrl = extraParams?.repoUrl || settings['git.repoUrl'];
  let branch = extraParams?.branch || settings['git.branch'] || 'main';
