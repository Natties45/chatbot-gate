...
│   ├── agents/prompts from apps/web1/gate-answer/
...
| `apps/web1/Dockerfile` | Node:22 Debian Bookworm (not Alpine) + Next.js build |
...
### apps/web1/Dockerfile
```dockerfile
FROM node:22 AS base
WORKDIR /app
COPY package*.json ./
COPY apps/web1/package*.json apps/web1/tsconfig.json ./apps/web1/
RUN npm install
COPY . .
FROM base AS prod
...
services:
  ...
  web1:
    build:
      context: .
      dockerfile: apps/web1/Dockerfile
      target: prod
    environment:
      - OPENCODE_SERVER_URL=http://opencode:4096
      - NODE_ENV=production
...
| `/opt/chatbot-gate/apps/web1/opencode.json` | Agent configs |
| `/opt/chatbot-gate/apps/web1/gate-answer/` | Prompts and templates |
